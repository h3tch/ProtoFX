...
namespace App
{
    class CodeCompletion : AutocompleteMenu
    {
        public CodeCompletion(Scintilla scintilla)
        {
            TargetControlWrapper = new ScintillaWrapper(scintilla);
            SetAutocompleteItems(new DynamicCollection(scintilla));
        }

        internal class DynamicCollection : IEnumerable<string>
        {
            ...
            {
                foreach (var word in SelectKeywords())
                    yield return word;
            }
            ...
            {
                string block = GetSouroundingBlock();
                string prev = GetPreviousWord();
                string cur = GetCurrentWord();
                ...
                if (block != null)
                {
                    ...
                    if (prev != null)
                    {
                        search = $"{block}.{prev}.{cur}";
                        result = from x in keywords
                                 where x.StartsWith(search)
                                 select x.Substring(search.Length - cur.Length);
                    }
                ...
                else
                {
                    ...
                    if (result == null || result.Count() == 0)
                    {
                        search = $"{cur}";
                        result = from x in keywords
                                 where x.IndexOf('.') < 0 && x.IndexOf(',') < 0 && x.StartsWith(cur)
                                 select x;
                    }
                }
            ...
            private string GetSouroundingBlock()
            {
                return null;
            }
